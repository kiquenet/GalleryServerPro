...
namespace GalleryServerPro.Web.Controller
{
	...
	public static class GalleryController
	{
		...
		public static void BeginSync(SyncOptions syncOptions)
		{
			...
			try
			{
				album = AlbumController.LoadAlbumInstance(syncOptions.AlbumIdToSynchronize, true, true, true);
		...
		private static void ValidateEncryptionKey()
		{
			...
			{
				AppSetting.Instance.Save(null, null, null, null, Utils.GenerateNewEncryptionKey(), null, null, null, null, null, null, null, null, null, null, null, null, null, null);
			}
		}
		...
		/// <summary>
		/// Handles the <see cref="GalleryServerPro.Business.Gallery.GalleryCreated" /> event.
		/// </summary>
		...
		/// <summary>
		/// Handles the <see cref="GalleryServerPro.Business.GallerySettings.GallerySettingsSaved" /> event.
		/// </summary>
		/// <param name="sender">The sender.</param>
		...
		private static void CreateSampleObjects(int galleryId)
		{
			...
			if (sampleAlbum == null)
			{
				...
				sampleAlbum.Caption = Resources.GalleryServerPro.Site_Welcome_Msg;
			...
			var sampleImage = sampleAlbum.GetChildGalleryObjects(GalleryObjectType.Image)
				.FirstOrDefault(image => image.Original.FileName == Constants.SAMPLE_IMAGE_FILENAME);
			...
			if (sampleImage == null)
			{
				...
				var sampleImageFilepath = Path.Combine(sampleDirPath, Constants.SAMPLE_IMAGE_FILENAME);

				if (!File.Exists(sampleImageFilepath))
				{
					var asm = Assembly.GetExecutingAssembly();
					using (var stream = asm.GetManifestResourceStream(String.Concat("GalleryServerPro.Web.App_GlobalResources.", Constants.SAMPLE_IMAGE_FILENAME)))
					{
						if (stream != null)
						{
							using (var fileStream = File.Create(sampleImageFilepath))
							{
				...
				if (File.Exists(sampleImageFilepath))
				{
					...
					var image = Factory.CreateImageInstance(new FileInfo(sampleImageFilepath), sampleAlbum);
					image.Title = "Margaret, Skyler and Roger Martin";
		...
		private static void UpdateRootAlbumTitleAfterAdminCreation(int galleryId)
		{
			...
			HelperFunctions.RemoveCache(CacheItem.Albums);
		}
