	...
	public static class UserController
	{
		...
		public static IUserAccountCollection GetUsersCurrentUserCanView(bool userIsSiteAdmin, bool userIsGalleryAdmin)
		{
			...
			else if (userIsGalleryAdmin)
			{
				...
				var usersCache = (ConcurrentDictionary<string, IUserAccountCollection>)HelperFunctions.GetCache(CacheItem.UsersCurrentUserCanView);
				...
				if (System.Web.HttpContext.Current.Session != null)
				{
					lock (usersCache)
					{
						...
						{
							usersCache.TryAdd(cacheKeyName, users);
						}
					}
		...
		public static User GetUserEntity(string userName, int galleryId)
		{
			...
			if (String.IsNullOrWhiteSpace(userName))
				return new Entity.User() { IsNew = true, GalleryId = galleryId, Roles = new string[0] };
			...
			var userAddPerms = SecurityManager.GetUserAddObjectPermissions(RoleController.GetGalleryServerRolesForUser(), galleryId);
			...
			return new Entity.User()
				{
					...
					CanAddAlbumToAtLeastOneAlbum = userAddPerms.Item1,
					CanAddMediaToAtLeastOneAlbum = userAddPerms.Item2,
					...
					Roles = RoleController.GetRolesForUser(userName),
					Password = null,
		...
		/// password, and roles are persisted. To save other properties, call <see cref="SaveUser(Entity.User, out string)" /> after executing this function.
		/// </summary>
		...
		private static bool UserHasBeenModified(IUserAccount userToSave, MembershipUser userInDb)
		{
			if (userToSave == null)
