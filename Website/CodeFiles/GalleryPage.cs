...
using Album = GalleryServerPro.Business.Album;
...
namespace GalleryServerPro.Web.Pages
{
	...
	public abstract class GalleryPage : UserControl
	{
		...
		/// <summary>
		/// Gets a value indicating whether the download links/embed code button is visible above a media object. This value is retrieved from the 
		/// <see cref="Gallery.ShowUrlsButton" /> property if specified; if not, it defaults to <c>true</c>.
		...
		public bool ShowUrlsButton
		{
			get
			{
				if (!_showUrlsButton.HasValue)
				{
					this._showUrlsButton = this.GalleryControl.ShowUrlsButton.GetValueOrDefault(true);
				}

				return this._showUrlsButton.Value;
			}
		}
		...
		public bool ShowSlideShowButton
		{
			get
			{
				...
				{
					this._showSlideShowButton = this.GalleryControl.ShowSlideShowButton.GetValueOrDefault(this.GallerySettings.EnableSlideShow);
				}
		...
		public SlideShowType SlideShowType
		{
			get
			{
				...
				{
					var ssType = this.GalleryControl.SlideShowType;
					this._slideShowType = (ssType != SlideShowType.NotSet ? ssType : this.GallerySettings.SlideShowType);
				}
		...
		public bool ShowTransferMediaObjectButton
		{
			get
			{
				...
				{
					this._showTransferMediaObjectButton = this.GalleryControl.ShowTransferMediaObjectButton.GetValueOrDefault(true);
				}
		...
		public bool ShowCopyMediaObjectButton
		{
			get
			{
				...
				{
					this._showCopyMediaObjectButton = this.GalleryControl.ShowCopyMediaObjectButton.GetValueOrDefault(true);
				}
		...
		public bool ShowRotateMediaObjectButton
		{
			get
			{
				...
				{
					this._showRotateMediaObjectButton = this.GalleryControl.ShowRotateMediaObjectButton.GetValueOrDefault(true);
				}
		...
		public bool ShowDeleteMediaObjectButton
		{
			get
			{
				...
				{
					this._showDeleteMediaObjectButton = this.GalleryControl.ShowDeleteMediaObjectButton.GetValueOrDefault(true);
				}
		...
		/// <summary>
		/// Gets the Gallery Server Pro logo.
		/// </summary>
		/// <value>An instance of <see cref="LiteralControl" />.</value>
		protected LiteralControl GspLogo
		{
			get
			{
				var tooltip = String.Format(CultureInfo.CurrentCulture, Resources.GalleryServerPro.Footer_Logo_Tooltip, Utils.GetGalleryServerVersion());

				return new LiteralControl(String.Format(CultureInfo.InvariantCulture, @"<footer class='gsp_addtopmargin5 gsp_footer'>
			  <a href='http://www.galleryserverpro.com' title='{0}'>
			   <img src='{1}' alt='{0}' />
			  </a>
			 </footer>",
																								tooltip,
																								Page.ClientScript.GetWebResourceUrl(this.GetType().BaseType, "GalleryServerPro.Web.App_GlobalResources.gsp-ftr-logo-170x46.png")));
			}
		}
		...
		private void EvaluateUserPermissions()
		{
			...
			this._userCanViewAlbumOrMediaObject = isPhysicalAlbum && this.IsUserAuthorized(SecurityActions.ViewAlbumOrMediaObject);
			this._userCanViewOriginal = isPhysicalAlbum && this.IsUserAuthorized(SecurityActions.ViewOriginalMediaObject);
			...
			this._userDoesNotGetWatermark = isPhysicalAlbum && this.IsUserAuthorized(SecurityActions.HideWatermark);
			...
			{
				var userAddPerms = SecurityManager.GetUserAddObjectPermissions(GetGalleryServerRolesForUser(), GalleryId);

				this._userCanAddAlbumToAtLeastOneAlbum = userAddPerms.Item1;
				this._userCanAddMediaObjectToAtLeastOneAlbum = userAddPerms.Item2;
			}
		}
		...
		//	writer.AddAttribute(HtmlTextWriterAttribute.Src, Page.ClientScript.GetWebResourceUrl(this.GetType().BaseType, "GalleryServerPro.Web.App_GlobalResources.gsp-ftr-logo-170x46.png"));
		...
		private void AddCssFiles(HtmlHead head)
		{
			if (head == null)
	...
	",
	 ...
	 rssUrl);
		...
		private void AddStartupScript()
		{
			...
			// inside a jsRender template or "$('.infoMsg', <%=cid%>.p())" in an ASCX page.
			string script = String.Format(CultureInfo.InvariantCulture, @"
	window.{0} = {{ }};
	window.{0}.p = function () {{ return $('#{0}'); }};
	window.{0}.gspData = $.parseJSON('{1}');
	window.{0}.gspData.ActiveMetaItems = (window.{0}.gspData.MediaItem ? window.{0}.gspData.MediaItem.MetaItems : window.{0}.gspData.Album.MetaItems) || [];
	window.{0}.gspData.ActiveGalleryItems = (window.{0}.gspData.MediaItem ? [window.Gsp.convertMediaItemToGalleryItem(window.{0}.gspData.MediaItem)] : [window.Gsp.convertAlbumToGalleryItem(window.{0}.gspData.Album)]) || [];
	{2}
",
		GspClientId, // 0
		GetClientGspDataAsJson(), // 1
		...
		{
			string script = String.Format(CultureInfo.InvariantCulture, @"
	window.Gsp.AppRoot = '{0}';
	window.Gsp.GalleryResourcesRoot = '{1}';
	window.Gsp.IsPostBack = {2};
",
		Utils.AppRoot, // 0
		Utils.GalleryResourcesPath, // 1
		IsPostBack.ToString().ToLowerInvariant() // 2
		);

			this.Page.ClientScript.RegisterStartupScript(this.GetType(), String.Concat(this.cid, "_initGlblScript"), script, true);
		}

		private string GetClientGspDataAsJson()
		{
			return GetClientGspData().ToJson().JsEncode();
		}
		...
		private string GetAlbumTreeDataClientScript()
		{
			...
			{
				return String.Format(CultureInfo.InvariantCulture, @"window.{0}.gspAlbumTreeData = $.parseJSON('{1}');", GspClientId, GetAlbumTreeDataAsJson());
			}
		...
		private string GetTitleUrlTooltip()
		{
			...
			if (!String.IsNullOrWhiteSpace(GalleryTitle) && (url.Length > 0))
			{
				switch (url)
				{
					case "/":
						{
							return Resources.GalleryServerPro.Header_PageHeaderTextUrlToolTipWebRoot;
						}
					case "~/":
						{
							return Resources.GalleryServerPro.Header_PageHeaderTextUrlToolTipAppRoot;
						}
					default:
						{
							return String.Format(CultureInfo.CurrentCulture, Resources.GalleryServerPro.Header_PageHeaderTextUrlToolTip, url);
						}
				}
			}
			else
		...
		private ClientMessageOptions GetMessageOptions(MessageType messageId)
		{
			...
			string title = Resources.GalleryServerPro.ResourceManager.GetString(String.Concat(resourcePrefix, messageId.ToString(), headerSuffix)) ?? String.Empty;
			string msg = Resources.GalleryServerPro.ResourceManager.GetString(String.Concat(resourcePrefix, messageId.ToString(), detailSuffix)) ?? String.Empty;
				...
				case MessageType.ObjectsSkippedDuringUpload:
					{
						...
						if (uploadResults != null)
						{
							...
							{
								string detailMsgTemplate = Resources.GalleryServerPro.ResourceManager.GetString(String.Concat(resourcePrefix, messageId.ToString(), "Single", detailSuffix)) ?? String.Empty;
							...
							{
								string detailMsgTemplate = Resources.GalleryServerPro.ResourceManager.GetString(String.Concat(resourcePrefix, messageId.ToString(), "Multiple", detailSuffix)) ?? String.Empty;
		...
		private MessageStyle GetMessageStyle(MessageType messageId)
		{
			switch (messageId)
			{
				...
				case MessageType.ThumbnailSuccessfullyAssigned:
				...
				case MessageType.CannotDeleteOriginalFilesNoObjectsExistInAlbum:
				case MessageType.CannotDeleteObjectsNoObjectsExistInAlbum:
				case MessageType.MediaObjectDoesNotExist:
				case MessageType.AlbumDoesNotExist:
					return MessageStyle.Info;

				case MessageType.ObjectsSuccessfullyDeleted:
				case MessageType.OriginalFilesSuccessfullyDeleted:
					return MessageStyle.Success;
				...
				case MessageType.CannotOverlayWatermarkOnImage:
				case MessageType.CannotRotateObjectNotRotatable:
					return MessageStyle.Info;

				case MessageType.ObjectsSuccessfullyMoved:
				case MessageType.ObjectsSuccessfullyCopied:
				case MessageType.ObjectsSuccessfullyRearranged:
				case MessageType.ObjectsSuccessfullyRotated:
					return MessageStyle.Success;

				case MessageType.ObjectsSkippedDuringUpload:
				case MessageType.CannotRotateInvalidImage:
				case MessageType.CannotEditGalleryIsReadOnly:
				case MessageType.CannotDownloadObjectsNoObjectsExistInAlbum:
					return MessageStyle.Info;
...
(function ($) {{
	$(document).ready(function () {{
		$.gspShowMsg('{0}', '{1}', {{msgType: '{2}', autoCloseDelay: {3}}});
	}});
}})(jQuery);
",
