...
namespace GalleryServerPro.Web
{
  ...
  public class Gallery : UserControl
  {
    ...
    /// <summary>
		/// Gets or sets a value indicating whether the download links/embed code button is visible above a media object. If the 
    /// property has not been explicitly assigned a value, it returns the value of <see cref="IGalleryControlSettings.ShowUrlsButton" />.
    ...
    /// </summary>
		/// <value><c>true</c> if the download links/embed code button is visible above a media object; otherwise, <c>false</c>.</value>
		public bool? ShowUrlsButton
    {
      get
      {
        return GalleryControlSettings.ShowUrlsButton;
      }
      set
      {
        GalleryControlSettings.ShowUrlsButton = value;
      }
    }
    ...
    private void LoadRequestedPage()
    {
      ...
      {
        throw new WebException(String.Format(CultureInfo.InvariantCulture, Resources.GalleryServerPro.Error_Cannot_Load_User_Control_Ex_Msg, src));
      }
    }
