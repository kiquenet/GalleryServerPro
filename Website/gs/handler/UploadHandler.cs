	...
	public class UploadHandler : UploadHandlerBase
	{
		...
		protected override bool OnUploadChunk(Stream chunkStream, int chunk, int chunks, string uploadedFilename)
		{
			...
			if (!Directory.Exists(path))
			{
				...
				{
					WriteErrorResponse(Resources.GalleryServerPro.Task_Add_Objects_UploadDirectoryDoesntExistAndCouldntCreate);
					return false;
				}
			}
			...
			{
				WriteErrorResponse(Resources.GalleryServerPro.Task_Add_Objects_UnableToWriteOutFile);
				return false;
			}
			finally
			{
