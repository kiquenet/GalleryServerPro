					...
					<section class="gsp_checkboxOptions">
						...
						<p id="P1" runat="server">
							<a id="gsp_chkDeleteUsers" class="gsp_disabled">
								<asp:Literal ID="l5" runat="server" Text="<%$ Resources:GalleryServerPro, Admin_Manage_Users_Delete_Selected_Users_Lbl %>" />
					...
					if ($(this).prop("checked")) {
						...
						$("#gsp_chkDeleteUsers").click(function (e) {
							...
							var ajaxArray = $.map(usersToDelete, function(userName, idx) {
								return $.ajax({
									type: "DELETE",
									url: window.Gsp.AppRoot + '/api/users/deletebyusername?userName=' + encodeURI(userName),
									error: function(response) {
										$.gspShowMsg("User '" + userName + "' not deleted", response.responseText, { msgType: 'error', autoCloseDelay: 0 });
							...
							$.when.apply($, ajaxArray).then(function() {
								...
								$.gspShowMsg("Users Deleted", "The users were successfully deleted.", { msgType: 'success' });
								onUsersDeleted();
			...
			var bindUserList = function (users) {
				...
				if (users == null) {
					if (gspUsers.length < <%= MaxNumberOfUsersToRender %>) {
						users = window.Gsp.deepCopy(gspUsers); // If no users have been specified default to full list
					} else {
						users = new Array();
					}
				}
				...
				users.splice(0, 0, '<%= Resources.GalleryServerPro.Admin_Manage_Users_Add_User_Link_Text %>');
