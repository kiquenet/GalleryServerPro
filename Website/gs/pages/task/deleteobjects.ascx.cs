...
namespace GalleryServerPro.Web.Pages.Task
{
	...
	public partial class deleteobjects : Pages.TaskPage
	{
		...
		private IList<IGalleryObject> _galleryObjects;
		...
		private IList<IGalleryObject> GalleryObjects
		{
			get { return this._galleryObjects ?? (this._galleryObjects = this.GetAlbum().GetChildGalleryObjects(GalleryObjectType.All).ToSortedList()); }
		}
		...
		protected override bool OnBubbleEvent(object source, EventArgs args)
		{
			...
			if ((btn != null) && (((btn.ID == "btnOkTop") || (btn.ID == "btnOkBottom"))))
			{
				if (btnOkClicked())
				{
					this.RedirectToAlbumViewPage("msg={0}", ((int)MessageType.ObjectsSuccessfullyDeleted).ToString(CultureInfo.InvariantCulture));
				}
			}
		...
		protected string GetGalleryObjectText(string title, Type galleryObjectType)
		{
			...
			if (galleryObjectType == typeof(Album))
			{
				...
				maxLength = GallerySettings.MaxThumbnailTitleDisplayLength;
			}
		...
		private void ConfigureControls()
		{
			...
			{
				this.RedirectToAlbumViewPage("msg={0}", ((int)MessageType.CannotDeleteObjectsNoObjectsExistInAlbum).ToString(CultureInfo.InvariantCulture));
			}
		}
