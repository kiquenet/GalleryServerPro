...
namespace GalleryServerPro.Web.Pages.Task
{
	...
	public partial class transferobject : Pages.TaskPage
	{
		...
		{
			if (GallerySettings.MediaObjectPathIsReadOnly && UserController.GetGalleriesCurrentUserCanAdminister().All(g => g.GalleryId == this.GalleryId))
			{
				// If the current gallery is read only and the user does not have admin privileges to any other gallery, bounce user out.
		...
		private TransferType TransType
		{
			get
			{
				if (this._transferType == TransferType.None)
				{
					...
					//else
					//  throw new GalleryServerPro.Events.CustomExceptions.UnexpectedQueryStringException();
				}
		...
		private IAlbum DestinationAlbum
		{
			get
			{
				...
				{
					this._destAlbum = tvUC.SelectedAlbum;
				}
		...
		protected string CheckCheckAllCtrCssClass
		{
			get
			{
				...
				case TransferObjectState.ObjectsCopyStep1:
					{
						...
						var albumChildren = this.GetAlbum().GetChildGalleryObjects().ToSortedList();

						if (!albumChildren.Any())
							this.RedirectToAlbumViewPage("msg={0}", ((int)MessageType.CannotCopyNoObjectsExistInAlbum).ToString(CultureInfo.InvariantCulture));
				...
				case TransferObjectState.ObjectsMoveStep1:
					{
						...
						if (!albumChildren.Any())
							this.RedirectToAlbumViewPage("msg={0}", ((int)MessageType.CannotMoveNoObjectsExistInAlbum).ToString(CultureInfo.InvariantCulture));
		...
		{
			MessageType msg = (this.TransType == TransferType.Copy ? MessageType.ObjectsSuccessfullyCopied : MessageType.ObjectsSuccessfullyMoved);
		...
		private void TransferObjects()
		{
			...
			try
			{
				...
				Factory.LoadAlbumInstance(DestinationAlbum.Id, false, true).Sort(true, Utils.UserName);
		...
		private void ValidateSecurityForAlbumOrMediaObject(IGalleryObject galleryObjectToMoveOrCopy, SecurityActions securityActions)
		{
			if (this.TransType == TransferType.Move)
			{
				if (!IsUserAuthorized(securityActions, galleryObjectToMoveOrCopy.Id, this.GalleryId, ((IAlbum)galleryObjectToMoveOrCopy).IsVirtualAlbum))
				{
