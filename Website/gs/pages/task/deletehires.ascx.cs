	...
	public partial class deletehires : Pages.TaskPage
	{
		...
		protected override bool OnBubbleEvent(object source, EventArgs args)
		{
			...
			if ((btn != null) && (((btn.ID == "btnOkTop") || (btn.ID == "btnOkBottom"))))
			{
				if (btnOkClicked())
				{
					this.RedirectToAlbumViewPage("msg={0}", ((int)MessageType.OriginalFilesSuccessfullyDeleted).ToString(CultureInfo.InvariantCulture));
				}
			}
		...
		{
			get { return this._galleryObjects ?? (this._galleryObjects = this.GetAlbum().GetChildGalleryObjects().ToSortedList()); }
		}
		...
		private void ConfigureControls()
		{
			...
			{
				this.RedirectToAlbumViewPage("msg={0}", ((int)MessageType.CannotDeleteOriginalFilesNoObjectsExistInAlbum).ToString(CultureInfo.InvariantCulture));
			}
		}
