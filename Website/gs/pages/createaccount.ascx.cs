...
namespace GalleryServerPro.Web.Pages
{
	...
	public partial class createaccount : Pages.GalleryPage
	{
		...
		private void ConfigureControlsFirstTime()
		{
			if (ViewMode == PageMode.CreateAdmin)
			{
				litHeader.Text = Resources.GalleryServerPro.CreateAccount_HeaderAdmin;
  ...
  <section class='gsp_ca_eula_logo_ctr gsp_roundedtop10'>
    <img src='{0}' alt='Gallery Server Pro logo' />
  </section>
...
", 
				Utils.GetSkinnedUrl("/images/gsp-logo.png"),
				Utils.GalleryRoot);
...
(function ($) {{
	$(document).ready(function () {{
		...
		$('.gsp_ca_eula_dg', $('#{0}')).dialog({{
			...
			modal: false,
			hide: 'fade',
			dialogClass: 'gsp_ca_eula_dg_ctr',
			buttons: [{{
				text: 'Accept',
				click: function() {{
					$(this).dialog( 'close' );
...
",
				...
				ClientMessage = new ClientMessageOptions
					                {
						                ...
						                Message = Resources.GalleryServerPro.CreateAccount_Verification_Username_Not_Valid_Email_Text,
		...
		private IUserAccount AddUser()
		{
			...
			if (newUserPassword1 != newUserPassword2)
				throw new WebException(Resources.GalleryServerPro.Admin_CreateAccount_Passwords_Not_Matching_Error);
