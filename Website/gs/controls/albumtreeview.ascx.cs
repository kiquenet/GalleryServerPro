...
using TreeView = GalleryServerPro.Web.Entity.TreeView;
...
namespace GalleryServerPro.Web.Controls
{
  ...
  public partial class albumtreeview : GalleryUserControl
  {
    ...
    /// <summary>
    /// During a postback, gets the album selected by the user or, during an initial page load, the album assigned to the first item
    ...
    public IAlbum SelectedAlbum
    {
      get
      {
        ...
        if (SelectedAlbumIds.Count == 1)
        {
          try
          {
            return AlbumController.LoadAlbumInstance(SelectedAlbumIds[0], false);
          }
