...
namespace GalleryServerPro.Business
{
	...
	public class ImageOriginalCreator : DisplayObjectCreator
	{
		...
		/// </summary>
		/// <exception cref="UnsupportedImageTypeException">Thrown when Gallery Server Pro cannot process the image, 
		...
		public override void GenerateAndSaveFile()
		{
			...
			// occur for existing objects.
			if ((GalleryObject.IsNew) || (GalleryObject.Rotation == MediaObjectRotation.NotSpecified))
				return;
			...
			var rotateResult = Rotate(filePath, GallerySettings.OriginalImageJpegQuality);

			if (rotateResult.Item1 > MediaObjectRotation.Rotate0)
			{
				if (!rotateResult.Item2.IsEmpty)
				{
					GalleryObject.Original.Width = (int)rotateResult.Item2.Width;
					GalleryObject.Original.Height = (int)rotateResult.Item2.Height;
				}
