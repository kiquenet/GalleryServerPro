...
namespace GalleryServerPro.Business
{
	...
	public class Album : GalleryObject, IAlbum
	{
		...
		internal Album(int albumId, int galleryId)
			: this(albumId, galleryId, int.MinValue, string.Empty, int.MinValue, MetadataItemName.NotSpecified, null, int.MinValue, DateTime.MinValue, DateTime.MinValue, String.Empty, DateTime.Now, String.Empty, DateTime.MinValue, String.Empty, String.Empty, false, false, null)
		{
		}
		...
		public void Sort(bool persistToDataStore, string userName, bool isRecursive = false, bool replaceChildSortFields = false)
		{
			...
			if (persistToDataStore)
			{
				...
				foreach (var go in GetChildGalleryObjects())
				{
					...
					go.DateLastModified = DateTime.Now;
		...
		public override IGalleryObject CopyTo(IAlbum destinationAlbum, string userName)
		{
			...
			albumCopy.IsPrivate = destinationAlbum.IsPrivate;
		...
		/// </summary>
		/// <exception cref="GalleryServerPro.Events.CustomExceptions.BusinessException">Thrown when the DirectoryName
		...
		private void SortAsyncBegin(bool persistToDataStore, string userName, bool isRecursive, bool replaceChildSortFields)
		{
			...
			catch (Exception ex)
			{
				...
				EventController.RecordEvent(String.Format(CultureInfo.CurrentCulture, "CANCELLED: The sorting of album '{0}' has been cancelled due to the previously logged error.", Id), EventType.Info, GalleryId, Factory.LoadGallerySettings(), AppSetting.Instance);
				throw;
			}
