...
namespace GalleryServerPro.Business
{
	...
	public class MediaObjectSaveBehavior : ISaveBehavior
	{
		...
		//	if (this._galleryObject.MetadataItems.TryGetMetadataItem(MetadataItemName.Title, out metaItem))
		//	{
		//		metaItem.Value = this._galleryObject.Title;
		//	}
		//}
		...
		/// does not exist (such as for external media objects).
		/// </summary>
		private void SyncFilenameToMetadataFilename()
		{
			if (Factory.LoadGallerySetting(_galleryObject.GalleryId).MediaObjectPathIsReadOnly)
			{
			...
			if (this._galleryObject.MetadataItems.TryGetMetadataItem(MetadataItemName.FileName, out metaItem))
			{
				if (!this._galleryObject.Original.FileName.Equals(metaItem.Value, StringComparison.OrdinalIgnoreCase))
				{
					...
					if (File.Exists(prevPath))
					{
						try
						{
							File.Move(prevPath, this._galleryObject.Original.FileNamePhysicalPath);
						}
						catch (IOException ex)
						{
