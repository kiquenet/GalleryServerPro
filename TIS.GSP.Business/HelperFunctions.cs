...
namespace GalleryServerPro.Business
{
	...
	public static class HelperFunctions
	{
		...
		public static string Encrypt(string plainText)
		{
			...
			byte[] stringToEncryptArray = Encoding.UTF8.GetBytes(plainText);
		...
		/// <summary>
		/// Remove all items from cache. This includes media objects, albums, gallery server roles, application errors, gallery settings, and more.
		/// </summary>
		public static void PurgeCache()
		{
			...
			RemoveCache(CacheItem.AppEvents);
			RemoveCache(CacheItem.Profiles);
			RemoveCache(CacheItem.UiTemplates);
			RemoveCache(CacheItem.MediaTemplates);
			RemoveCache(CacheItem.MimeTypes);

			Factory.ClearGalleryCache(); // Since galleries store a list of all albums, we must clear it out anytime an album is added, deleted, or moved.
		}
		...
		private static byte[] EncryptionKey
		{
			get
			{
				...
				{
					_encryptionKey = Encoding.UTF8.GetBytes(AppSetting.Instance.EncryptionKey);
				}
