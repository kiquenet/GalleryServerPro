...
namespace GalleryServerPro.Data
{
	...
	public static class Migrate26Controller
	{
		...
		/// </summary>
		/// <param name="ds">The DataSet.</param>
		private static void MigrateAppSettings(DataSet ds)
		{
			...
			foreach (DataRow rSource in dtSource.Rows)
			{
				...
				foreach (DataColumn col in dtSource.Columns)
				{
					switch (col.ColumnName)
					{
						...
						case "DateAdded":
							rDest[col.ColumnName] = DateTime.Now;
							break;
		...
		/// <summary>
		/// Copies the contents of <see cref="dtSource" /> into <see cref="dtDest" />. Requires that the structure be the same
